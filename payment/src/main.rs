...
impl<'a> RabbitEnv<'a> {
    fn new(host: &'a str, username: &'a str, password: &'a str, port: &'a str) -> Self {
        Self {
            host,
            username,
            password,
...
fn main() {
    let rabbitmq_queue_payment =
        env::var("RABBITMQ_QUEUE_PAYMENT").unwrap_or_else(|_| "payment_queue".to_string());

    let env_props = RabbitEnv::new(
        "RABBITMQ_HOST",
        "RABBITMQ_USERNAME",
        "RABBITMQ_PASSWORD",
        "RABBITMQ_PORT",
    ...
    payment_queue
        .lister(&rabbitmq_queue_payment, listen_payment)
        .unwrap();
}
...
fn listen_payment(message: String) {
    ...
    order.status = Status::from(random_value);
    {
        ...
        let rabbitmq_queue_order =
            env::var("RABBITMQ_QUEUE_ORDER").unwrap_or_else(|_| "order_queue".to_string());
...
fn build_dns(env_props: RabbitEnv) -> String {
    let rabbitmq_host = env::var(env_props.host).unwrap_or_else(|_| "localhost".to_string());
    let rabbitmq_username = env::var(env_props.username).unwrap_or_else(|_| "guest".to_string());
    let rabbitmq_password = env::var(env_props.password).unwrap_or_else(|_| "guest".to_string());
    let rabbitmq_port: usize = env::var(env_props.port)
        .map(|val| val.parse().unwrap_or(5672))
        .unwrap_or_else(|_| 5672);
